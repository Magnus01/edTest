...
return data.map((node) => {
    ...
    for (let i = 0; i < this.props.inclass.chapteranalytics.length; i++) {
        if (node.id == this.props.inclass.chapteranalytics[i][0]) {
            let average = [];
            let number = 0;
            for (let j = 0; j < JSON.parse(this.props.inclass.chapteranalytics[i][2])['user-list'].length; j++) {



                if (typeof JSON.parse(this.props.inclass.chapteranalytics[i][2])['user-list'][j]['chapter-completion'] !== "undefined"|| JSON.parse(this.props.inclass.chapteranalytics[i][2])['user-list'][j]['chapter-completion'] !== null) {
                    let numbertopush = JSON.parse(this.props.inclass.chapteranalytics[i][2])['user-list'][j]['chapter-completion'];
                    // average.push(numbertopush);
                    // number =+ numbertopush;
                    if (!isNaN(numbertopush) ) {
                        number = (numbertopush || 0) + parseFloat(numbertopush)
                        console.log(number, 'number');
                    }
                    // console.log(average, 'average');

                }
               // let chapteraverage = arrSum(average)/average.length;
               //  console.log(chapteraverage)
                // console.log(), 'lookyhere')
                //     // console.log(this.props.inclass.chapteranalytics[i][0])
                //     // node.percentage ==
                // }
                // console.log(node.id, this.props.inclass.chapteranalytics[i][0], 'this.props.inclass.chapteranalytics[i][0]')

            }
            console.log(number, 'number');
            console.log(JSON.parse(this.props.inclass.chapteranalytics[i][2])['user-list'].length, 'JSON.parse(this.props.inclass.chapteranalytics[i][2]');
            console.log( number/JSON.parse(this.props.inclass.chapteranalytics[i][2])['user-list'].length, 'divide');
            if (!isNaN(number) ) {
                node.percentage = number/JSON.parse(this.props.inclass.chapteranalytics[i][2])['user-list'].length;
            }
            // node.percentage === 1;
            console.log(node, 'noce');
            console.log(node.percentage, 'node.percentage');
...
if (node.children && node.children.length > 0) {
    ...
    //
    // // be careful the chapter id from the studentplatform api is not the same as teh chapter id in the contentapi
    //     node.percentage  === chapter.percentage
    // }
    //
    //
    //
